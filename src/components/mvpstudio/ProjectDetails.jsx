...
const ProjectDetails = () => {
  ...
  const projectList = projectsData[tab] || [];
  const project = projectList.filter(p => p.category === category)[idx];
  ...
  return (
    ...
    <section className="w-full mt-[60px] min-h-screen bg-[#f7fafd] px-2 py-8 flex flex-col items-center">
      ...
      >
        <SwiperSlide>
          <img src={project.img}
          alt=''  className='h-[50px] bg-gray-300 object-cover object-center w-full'/>
        </SwiperSlide>
        <SwiperSlide>
          <img src={project.img1}
          alt='' className='h-[50px] bg-gray-200'/>
        </SwiperSlide>
        <SwiperSlide>
          <img src={project.img2}
          alt='' className='h-[50px] bg-gray-100'/>
        </SwiperSlide>
        <SwiperSlide>
          <img src={project.img3}
          alt='' className='h-[50px] bg-gray-50'/>
        </SwiperSlide>
      </Swiper>
        </div>
        ...
        <div className="w-full md:w-1/2 flex flex-col justify-center md:items-start max-md:text-center">
          <h2 className="font-semibold text-xl md:text-2xl mb-2">{project.title}</h2>
          <p className="text-sm md:text-base mb-6 text-[#222]">{project.desc}</p>
          <a href={project.explorelink} target='_blank' className='w-full'><button className="bg-[#2563eb] text-white px-6 py-2 md:py-3 rounded-lg w-full font-medium text-sm hover:bg-[#1051FF] transition mb-4">
            Explore
          </button></a>
        </div>
      </div>
      ...
      <div className="w-full max-w-6xl mx-auto flex flex-col items-center">
        ...
        {project.experts && project.experts.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-2 xl:gap-4 w-full mb-8">
        ...
        <Link to='/mvpstudio'><button className="bg-[#2563eb] text-white px-6 py-2 rounded-lg font-medium text-sm hover:bg-[#1051FF] transition mb-4">
          See All &rarr;
        </button></Link>
